...
- **Next action:** **Waiting on the user** to run [the Phase 5 manual checks](../okf/runbooks/phase5-manual-checks.md) on their Mac. Nothing else is unblocked: Phase 8 (OpenClaw, optional) needs its API contract (open question 6), and Phase 9 (avatar) is locked until the MVP (Phases 1–5) is done.
...
- Phase 8 needs the OpenClaw API (open question 6), or an explicit decision to defer it (it's optional).
...
- ruff clean; OKF lint 0 errors; `phases_status.py` → phases 0–4, 6, 7 done; 5 in_progress 1/5.
