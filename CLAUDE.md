...
python scripts/nightly_update.py [--schedule]   # night cycle once (cron/launchd) or daily via apscheduler
...
- **Memory** ([ADR-0010](docs/okf/decisions/0010-memory-and-trust.md)): personal files live in `~/.synkage/memory/` (0700, `SYNKAGE_MEMORY_DIR`), created on every CLI run if missing: `user_profile.json`, `relationship_state.json` (trust), `logs.jsonl` (audit), `log_summaries.json`, `archive/`. The night cycle (`synkage/memory/night_cycle.py`) decays trust toward neutral, applies outcome deltas, and compresses old audit records losslessly. It is the audit log's only rewriter, and it shares an exclusive flock with `AuditLog.append`. Trust is not wired into autonomy yet and must never loosen hard rules. Tests use a temp memory dir (autouse fixture).
